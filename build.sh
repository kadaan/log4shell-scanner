...
function run() {
  ...
  ${GOX} -os="${XC_OS}" -arch="${XC_ARCH}" -osarch="!darwin/arm !darwin/arm64 !darwin/386" -tags 'osusergo netgo static_build' -ldflags "-s -w -extldflags \"-fno-PIC -static\" -X github.com/kadaan/log4shell-scanner/version.Version=$VERSION -X github.com/kadaan/log4shell-scanner/version.Revision=$REVISION -X github.com/kadaan/log4shell-scanner/version.Branch=$BRANCH -X github.com/kadaan/log4shell-scanner/version.BuildUser=$USER -X github.com/kadaan/log4shell-scanner/version.BuildHost=$HOST -X github.com/kadaan/log4shell-scanner/version.BuildDate=$BUILD_DATE" -output="dist/{{.Dir}}_{{.OS}}_{{.Arch}}" || fatal "gox failed: $?"
  ...
  if [[ -n "$TRAVIS" ]]; then
    ...
    for f in *; do
      ...
      verbose "   --> $archivename"
      local genericname="log4shell-scanner$extension"
      mv -f "$f" "$genericname"
